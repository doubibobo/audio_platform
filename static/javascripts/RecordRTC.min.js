...
// Last time updated: 2018-06-17 2:39:20 AM UTC
...
function RecordRTC(mediaStream, config) {
    function startRecording(config2) {
        return config2 && (config = new RecordRTCConfiguration(mediaStream, config2)), config.disableLogs || console.log("started recording " + config.type + " stream."), mediaRecorder ? (mediaRecorder.clearRecordedData(), mediaRecorder.record(), setState("recording"), self.recordingDuration && handleRecordingDuration(), self) : (initRecorder(function () {
    ...
    var mediaRecorder, self = this,
        ...
        returnObject = {
            ...
            destroy: function () {
                ...
                config = {
                    disableLogs: !0
                }, self.reset(), setState("destroyed"), returnObject = self = null, Storage.AudioContextConstructor && (Storage.AudioContextConstructor.close(), Storage.AudioContextConstructor = null), config.disableLogs = disableLogsCache, config.disableLogs || console.warn("RecordRTC is destroyed.")
            },
            version: "5.4.7"
        };
...
function MRecordRTC(mediaStream) {
    this.addStream = function (_mediaStream) {
        ...
        function processInWebWorker(_function) {
            var url, blob = URL.createObjectURL(new Blob([_function.toString(), "this.onmessage =  function (e) {" + _function.name + "(e.data);}"], {
                    type: "application/javascript"
                })),
                worker = new Worker(blob);
...
function setSrcObject(stream, element, ignoreCreateObjectURL) {
    if ("createObjectURL" in URL && !ignoreCreateObjectURL) try {
        element.src = URL.createObjectURL(stream)
    } catch (e) {
        return void setSrcObject(stream, element, !0)
    } else "srcObject" in element ? element.srcObject = stream : "mozSrcObject" in element ? element.mozSrcObject = stream : alert("createObjectURL/srcObject both are not supported.")
}

function isMediaRecorderCompatible() {
    var isOpera = !!window.opera || navigator.userAgent.indexOf(" OPR/") >= 0,
        isChrome = !!window.chrome && !isOpera || isElectron(),
...
function MediaStreamRecorder(mediaStream, config) {
    ...
    this.getArrayOfBlobs = function () {
        ...
        recorderHints.mimeType && !MediaRecorder.isTypeSupported && "canRecordMimeType" in mediaRecorder && mediaRecorder.canRecordMimeType(recorderHints.mimeType) === !1 && (config.disableLogs || console.warn("MediaRecorder API seems unable to record mimeType:", recorderHints.mimeType)), mediaRecorder.ondataavailable = function (e) {
            ...
            } else if (e.data && e.data.size && e.data.size > 100 && (arrayOfBlobs.push(e.data), updateTimeStamp(), "function" == typeof config.ondataavailable)) {
...
function StereoAudioRecorder(mediaStream, config) {
    ...
    function mergeLeftRightBuffers(config, callback) {
        ...
        if (isEdge || isOpera || isSafari || config.noWorker) return void mergeAudioBuffers(config, function (data) {
    ...
    if (legalBufferValues.indexOf(bufferSize) === -1 && (config.disableLogs || console.warn("Legal values for buffer-size are " + JSON.stringify(legalBufferValues, null, "\t"))), context.createJavaScriptNode) jsAudioNode = context.createJavaScriptNode(bufferSize, numberOfAudioChannels, numberOfAudioChannels);
    else {
    ...
    jsAudioNode.onaudioprocess = onAudioProcessDataAvailable, jsAudioNode.connect(context.destination), this.leftchannel = leftchannel, this.rightchannel = rightchannel, this.numberOfAudioChannels = numberOfAudioChannels, this.desiredSampRate = desiredSampRate, this.sampleRate = sampleRate, self.recordingLength = recordingLength;
...
function GifRecorder(mediaStream, config) {
    ...
    if ("undefined" == typeof GIFEncoder) {
        ...
        script.src = "https://cdn.webrtc-experiment.com/gif-recorder.js", (document.body || document.documentElement).appendChild(script)
    }
    config = config || {};
    ...
    if (!isHTMLObject) {
        ...
        video.muted = !0, video.autoplay = !0, isLoadedMetaData = !1, video.onloadedmetadata = function () {
            isLoadedMetaData = !0
...
function MultiStreamsMixer(arrayOfMediaStreams) {
    ...
    function getVideo(stream) {
        var video = document.createElement("video");
        return setSrcObject(stream, video), video.muted = !0, video.volume = 0, video.width = stream.width || self.width || 360, video.height = stream.height || self.height || 240, video.play(), video
    }
...
RecordRTC.version = "5.4.7", "undefined" != typeof module && (module.exports = RecordRTC), "function" == typeof define && define.amd && define("RecordRTC", [], function () {
    return RecordRTC
